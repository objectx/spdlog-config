...
namespace spdlog_config { namespace detail {
    spdlog::level::level_enum parse_log_level (const std::__1::string & s) {
        struct level_desc_t {
        ...
        const static std::__1::array<level_desc_t, 10>
        ...
        for (size_t i = 0; i < levels.size (); ++i) {
            ...
            if (strncasecmp (lv.c_str (), s.c_str (), std::__1::min (s.size (), lv.size ())) == 0) {
                if (lv.size () == s.size ()) {
                    // Found exact match
                ...
                if (found != nullptr) {
                    throw std::runtime_error {
                            std::__1::string { "Log level \"" }.append (s).append ("\" is ambiguous.") };
                }
                found = &levels[i];
            }
        }
        if (found == nullptr) {
            throw std::runtime_error { std::__1::string { "Unknown log level \"" }.append (s).append ("\" found.") };
        }
        return found->level_;
    }
}}